...
const PreviousCourse = ({route, navigation}) => {
  const [PreviousCourses, setPreviousCourse] = useState([]);
    ...
    const GetPreviousCourses = async () => {
      try {
        const response = await fetch(
          `${global.MyIpAddress}/ShowAllCoursesDetail`,
          {
            method: 'POST',
            headers: {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>Your All Courses</Text>
      <FlatList
